...
class DailyTask extends Controller {
    ...
    public function getMonthData() {
        ...
        $data["this_month"] = TaskType::selectRaw("count(tasks.id) as times,task_types.name,task_types.weight")
            ...
            ->groupBy("tasks.type_id")->where("tasks.mark", true)->orderBy("times", "desc")->get();
    ...
    public function getRecentlyExercise() {
        $bodyTypes = TaskType::where('classify', "body")->pluck("id");
        $data = Task::selectRaw("max(tasks.date) as date ,tasks.type_id,task_types.name")
            ->join("task_types", "task_types.id", "=", "tasks.type_id")
            ->whereIn("type_id", $bodyTypes)->where("mark", true)
            ->groupBy("type_id")->get();
        return $this->success($data);
    }
}
